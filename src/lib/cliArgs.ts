...
export function printCliHelp(): void {
  process.stdout.write(
    [
      ...
      "  polterbase app install uru --platform macos [--artifact-url <url>] [--install-dir <dir>] [--yes]",
      "",
      "Notes:",
      "  - App workflows stay separate from the generic Supabase interactive menu.",
      "  - `install uru` requires an artifact URL via --artifact-url or POLTERBASE_URU_MACOS_ARTIFACT_URL.",
      "",
    ].join("\n"),
  );
}
